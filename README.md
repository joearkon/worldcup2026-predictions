...
├── deploy.sh           # Cloudflare Pages 发布脚本（含 data.js 版本戳防缓存）
└── README.md
```
...
托管于 Cloudflare Pages（直传模式）。更新三连：

```bash
# 1. 改 data.js / reports/   2. 提交
git add -A && git commit -m "..." && git push
# 3. 发布到 Cloudflare Pages（脚本会把站点文件复制进干净目录再直传，并给 data.js 打时间戳防缓存）
bash deploy.sh
```

> 说明：Pages 与 GitHub 无自动联动，需手动跑 `deploy.sh`；`index.html` 每次回源校验、`data.js` 带部署时间戳，访客普通刷新即可拿到最新数据。
