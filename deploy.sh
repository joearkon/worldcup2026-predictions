...
cp index.html bracket.html road.html data.js theme.css theme.js _headers .deploy-dist/
...
# 第三方库（html2canvas / qrcode，夺冠之路海报用）
[ -d vendor ] && mkdir -p .deploy-dist/vendor && cp -r vendor/* .deploy-dist/vendor/
...
sed -i "s|src=\"data.js\"|src=\"data.js?v=$VER\"|g" .deploy-dist/index.html .deploy-dist/bracket.html .deploy-dist/road.html
echo "stamped data.js -> ?v=$VER (index.html + bracket.html)"
